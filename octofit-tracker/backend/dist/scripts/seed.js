...
async function seed() {
    ...
    const teamDocs = teamAgg.map((t, i) => ({ scope: 'team', refId: t._id, rank: i + 1, totalDuration: t.totalDuration, totalCalories: t.totalCalories }));
    const userDocs = userAgg.map((u, i) => ({ scope: 'user', refId: u._id, rank: i + 1, totalDuration: u.totalDuration, totalCalories: u.totalCalories }));
    ...
    try {
        ...
        console.log('API verification skipped or failed (is the backend server running?):', err.message);
    }
